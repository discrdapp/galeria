...
document.addEventListener('DOMContentLoaded', () => {
	...
	lewy.addEventListener('click', () => {
		card.style.transform = `translateX(-340px) rotate(-35deg)`
        ktoreTeraz--;
		if (ktoreTeraz > Object.keys(zdj).length) {
			ktoreTeraz = 1
		}
		if (ktoreTeraz < 1) {
			ktoreTeraz = Object.keys(zdj).length
		}
		wypelnijZdj()
        setTimeout(() => {
        card.style.transform = `translateX(0px) rotate(0deg)`            
    ...
    const pomocPrzycisk = document.getElementById("pomocPrzycisk");
    const pomoc = document.getElementById("pomoc");
    const zamknijPomoc = document.getElementById("zamknijPomoc");
    const przyciemnienie = document.getElementById("przyciemnienie");

    pomocPrzycisk.addEventListener("click", () => { 
        pomoc.style.display = 'block';
        przyciemnienie.style.display = 'block';
    })

    zamknijPomoc.addEventListener("click", () => {
        pomoc.style.display = 'none';
        przyciemnienie.style.display = 'none';
    })

    przyciemnienie.addEventListener("click", () => {
        pomoc.style.display = 'none';
        przyciemnienie.style.display = 'none';
    })
})
